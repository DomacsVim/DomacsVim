...
function M.about()
  ...
  local lines = {
    ...
    " Version : 1.0.0",
    " Commit  : " .. version(),
    " Date    : " .. string.format("%s", vim.fn.system("echo $(git -C " .. dvim_runtime_dir.. " log -1 --format=%cd)"):gsub("%s+", "")),
    " OS      : " .. string.format("%s", vim.fn.system("uname -srm"):gsub("%s+", "")),
    " Nvim    : " .. string.format("%s", vim.fn.system("nvim --version | grep -oP '(?<=^NVIM v)[0-9|.]+'"):gsub("%s+", "")),
